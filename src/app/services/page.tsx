...
const AUTOMATION_TIERS = [
  ...
  },
  {
    ...
    name: "Standard",
    price: "£600–1,200",
    desc: "Multi-step logic with conditions, 2–3 integrations.",
    examples: [
      ...
      <section className="relative py-16">
        ...
        <div className="mx-auto max-w-5xl px-6">
          <Reveal className="relative">
            <span className="absolute top-0 right-6 rounded-full bg-green-glow px-2.5 py-1 text-[0.7rem] font-medium text-green-light sm:right-10">
              Core service
            </span>
            ...
            <div className="mt-5 flex flex-col gap-4 max-w-2xl text-text-secondary">
              <p>
          ...
          <Reveal className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-3">
            {AUTOMATION_TIERS.map((t) => (
              <div
                key={t.name}
                className={`card-hover relative rounded-2xl border bg-bg-2 p-7 ${
                  t.highlight ? "border-green-light/40" : "border-border"
                }`}
              >
                {t.highlight && (
                  <span className="absolute top-7 right-7 rounded-full bg-green-glow px-2.5 py-1 text-[0.7rem] font-medium text-green-light">
                    Most common
                  </span>
                )}
                <span className="text-xs font-medium uppercase tracking-wider text-text-muted">
